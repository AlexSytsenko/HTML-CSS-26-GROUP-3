...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"themes/default/ug-theme-default.js":[function(require,module,exports) {
...
function UGTheme_default() {
  ...
  var g_functions = new UGFunctions(); //theme options
  ...
  var g_options = {
    ...
    //enable the panel text panel. 
    theme_text_padding_left: 20,
    //left padding of the text in the textpanel
    theme_text_padding_right: 5,
    //right paddin of the text in the textpanel
    theme_text_align: "left",
    //left, center, right - the align of the text in the textpanel
    theme_text_type: "description",
    //title, description, both - text that will be shown on the text panel, title or description or both
  ...
  var g_mustOptions = (_g_mustOptions = {
    slider_enable_text_panel: false,
    slider_enable_play_button: false,
    slider_enable_fullscreen_button: false
  }, _defineProperty(_g_mustOptions, "slider_enable_text_panel", false), _defineProperty(_g_mustOptions, "slider_textpanel_height", 50), _defineProperty(_g_mustOptions, "slider_textpanel_align", "top"), _g_mustOptions);
  var g_temp = {
    isPanelHidden: false
  };
  /**
  ...
  function initTheme(gallery, customOptions) {
    ...
    modifyOptions(); //set gallery options

    g_gallery.setOptions(g_options); //include gallery elements
    ...
    if (g_options.theme_load_slider == true) g_objSlider = g_objects.g_objSlider; //init text panel
  ...
  function setHtml() {
    ...
    if (!g_objTextPanel) {
      //take the buttons from default theme
    ...
    if (g_options.theme_enable_fullscreen_button == true) htmlAdd += "<div class='" + classButtonFullscreen + "'></div>"; //add play button to the panel

    if (g_options.theme_enable_play_button == true) htmlAdd += "<div class='" + classButtonPlay + "'></div>"; //add hide panel button
    ...
    htmlAdd += "</div>";
    g_objWrapper.append(htmlAdd); //set elements
    ...
    if (g_options.theme_enable_hidepanel_button == true) g_objButtonHidePanel = g_objPanel.children(".ug-default-button-hidepanel"); //set html strip panel to the panel

    g_objStripPanel.setHtml(g_objPanel); //set text panel html to the panel

    if (g_objTextPanel) g_objTextPanel.appendHTML(g_objPanel); //set slider html
  ...
  function initPanel() {
    ...
    var galleryWidth = objGallerySize.width; //init srip panel width

    g_objStripPanel.setOrientation("bottom");
    g_objStripPanel.setWidth(galleryWidth);
    g_objStripPanel.run(); //set panel size
    ...
    g_functions.setElementSize(g_objPanel, galleryWidth, panelHeight); //position strip panel

    var stripPanelElement = g_objStripPanel.getElement();
    g_functions.placeElement(stripPanelElement, "left", "bottom"); //init hide panel button

    if (g_objButtonHidePanel) {
      var buttonTip = g_objButtonHidePanel.children(".ug-default-button-hidepanel-tip");
      g_functions.placeElement(buttonTip, "center", "middle"); //set opacity and bg color from the text panel			
    ...
    if (g_objButtonHidePanel) {
      if (g_objTextPanel) //place at the beginning of hte panel
        g_functions.placeElement(g_objButtonHidePanel, "left", "top");else {
        //place above the strip panel
  ...
  function placeSlider() {
    ...
    var sliderWidth = g_gallery.getWidth(); //set parent container the panel
  ...
  function initEvents() {
    ...
    g_objGallery.on(g_gallery.events.GALLERY_BEFORE_REQUEST_ITEMS, onBeforeReqestItems); //set the panel buttons
  ...
  this.destroy = function () {
    ...
    g_objGallery.off(g_gallery.events.GALLERY_BEFORE_REQUEST_ITEMS); //set the panel buttons

    if (g_objButtonPlay) g_gallery.destroyPlayButton(g_objButtonPlay); //init fullscreen button

    if (g_objButtonFullscreen) g_gallery.destroyFullscreenButton(g_objButtonFullscreen); //init hide panel button
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","themes/default/ug-theme-default.js"], null)
//# sourceMappingURL=/ug-theme-default.ca1c0072.js.map